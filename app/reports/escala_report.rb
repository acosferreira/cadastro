class MembroReport < Ruport::Controller
  ...
  def setup
    conditions =["tipo_id= #{options.funcao} and r.id=#{options.reuniao}"]
    if options.funcao.empty?

      self.data=Pessoa.report_table_by_sql("select 'Escolha o grupo que deseja listar' from dual")

    else
      self.data= Pessoa.report_table_by_sql("select * from pessoas p ,
                                             reuniaos r, pessoas_reuniaos pr
                                            where #{conditions}
                                              and pr.pessoa_id = p.id and pr.reuniao_id= r.id
")
